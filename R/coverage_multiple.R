...
coverage_multiple <- function(data,
  ...
  #get index list thing
  #for loop (for each in length of list)
  sets <- data %>%
  ...
  for (j in 1:length(permutations)) {
    ...
    if (is.null(subset)) {
      out_of_coverage <- dsamp[[parameter]] < dsamp[[lower]] | dsamp[[parameter]] > dsamp[[upper]]
      ...
      for(i in 1:length(sets)) {
        ...
        #alternative to dplyr... tempdsamp <- dsamp[[dsamp[[subset]] == set,]]
        out_of_coverage <- tempdsamp[[parameter]] < tempdsamp[[lower]] | tempdsamp[[parameter]] > tempdsamp[[upper]]
        number_out <- out_of_coverage %>%
          as.numeric() %>%
          sum()
  ...
  median <- aggregate(dplyr::select(ld, -subset), by=fby, median) %>%
